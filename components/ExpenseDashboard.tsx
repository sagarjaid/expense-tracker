...
const ExpenseDashboard = React.forwardRef<{ refresh: () => void }, {}>(
  (props, ref) => {
    ...
    const [sortField, setSortField] = useState<
      "date" | "category" | "subcategory" | "amount" | "source"
    >("date");
    ...
    // Sorting function
    const handleSort = (
      field: "date" | "category" | "subcategory" | "amount" | "source"
    ) => {
      if (sortField === field) {
    ...
    const tabClass = (tab: "list" | "summary") =>
      `px-4 py-2 rounded-t-md text-sm cursor-pointer ${
        activeTab === tab
          ? "border border-b-0 font-medium border-border"
          : "border-border"
      }`;
                ...
                <div className="text-sm px-2 py-1 border border-border rounded-md bg-background text-foreground">
                  ...
                  <span className="font-bold text-foreground">
                    {(() => {
                      ...
                      const avgPerDay =
                        daysDiff > 0 ? totalAmount / daysDiff : 0;
                      ...
                      >
                        ...
                        <Legend
                          payload={mainCategories.map((cat) => {
                            const total = mainCatChartData.reduce(
                              (sum, day) => sum + (day[cat] || 0),
                              0
                            );
                            const grandTotal = mainCatChartData.reduce(
                              (sum, day) =>
                                sum +
                                (day.Needs || 0) +
                                (day.Wants || 0) +
                      ...
                      <table className="min-w-full text-sm">
                        <thead>
                          <tr className="border border-border">
                            ...
                            <th className="px-3 py-2 text-left whitespace-nowrap text-text-soft">
                              Description
                            </th>
                            <th
                            ...
                            <th className="px-3 py-2 text-center whitespace-nowrap text-text-soft">
                              Delete
                            </th>
                            <th className="px-3 py-2 text-center whitespace-nowrap text-text-soft">
                              Edit
                    ...
                    <div key={cat} className="">
                      ...
                      <div className="grid grid-cols-1 gap-2">
                        {mergedSubcategoriesRef.current[cat]
                          .filter(
                            (subcat) =>
                              !hideZero || (totals[cat]?.[subcat] || 0) > 0
                          )
                          .map((subcat: string) => {
                                        ...
                                        <table className="w-full text-sm">
                                          <thead>
                                            <tr className="bg-muted/50 border-b border-border">
                                              ...
                                              <th className="px-3 py-3 text-center font-medium w-12 text-foreground">
                                                Delete
                                              </th>
                                              <th className="px-3 py-3 text-center font-medium w-12 text-foreground">
                                                Edit
                                              </th>
                                              <th className="px-3 py-3 text-right font-medium w-[140px] text-foreground">
                                                Amount
                                              </th>
                                            </tr>
                                          </thead>
                      ...
                      <div className="mt-2 font-medium text-sm text-right text-text-soft">
                        ...
                        {mergedSubcategoriesRef.current[cat]
                          ...
                          .reduce(
                            (sum, subcat) => sum + (totals[cat]?.[subcat] || 0),
                            0
                          )
                          .toFixed(2)}
                      </div>
                            ...
                            (sum, cat) =>
                              ...
                              mergedSubcategoriesRef.current[cat]
                                .filter(
                                  (subcat) =>
                                    !hideZero ||
                                    (totals[cat]?.[subcat] || 0) > 0
                                )
                                .reduce(
                                  (a, subcat) =>
                                    a + (totals[cat]?.[subcat] || 0),
                                  0
                                ),
                            0
                          )
